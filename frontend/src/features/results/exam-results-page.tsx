...
export const ExamResultsPage = () => {
  ...
  return <div>
    ...
    <Card className="filter-bar filter-bar--results">
      ...
      <Select id="candidate-result-status" label="Attempt status" value={status} onChange={(event) => { setStatus(event.target.value as CandidateResultStatus | ''); setPage(0) }}><option value="">All statuses</option><option value="NOT_STARTED">Not started</option><option value="IN_PROGRESS">In progress</option><option value="SUBMITTED">Submitted</option><option value="AUTO_SUBMITTED">Auto-submitted</option></Select>
    ...
    {candidates.isPending ? <LoadingSkeleton /> : candidates.isError ? <EmptyState title="Candidate results could not be loaded" description={friendlyApiError(candidates.error)} /> : candidates.data.content.length === 0 ? <EmptyState title="No candidate results found" description="Adjust the search or result filters." /> : <>
      <Card className="responsive-data"><table><thead><tr><th>Candidate</th><th>Status</th><th>Score</th><th>Percentage</th><th>Outcome</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>
        {candidates.data.content.map((candidate) => <tr key={candidate.candidateId}>
          ...
          <td data-label="Status"><Badge tone={candidate.attemptStatus === 'IN_PROGRESS' ? 'info' : candidate.attemptStatus === 'NOT_STARTED' ? 'neutral' : 'success'}>{label(candidate.attemptStatus)}</Badge></td>
          ...
          <td data-label="Outcome">{candidate.passed === null ? 'Pending' : candidate.passed ? 'Passed' : 'Failed'}</td>
          <td className="row-actions">{candidate.attemptId ? <Button size="sm" variant="ghost" icon={<Eye size={15} />} onClick={() => navigate(`/institution/results/attempts/${candidate.attemptId}`)}>Review</Button> : <span>Not started</span>}</td>
        </tr>)}
      </tbody></table></Card>
